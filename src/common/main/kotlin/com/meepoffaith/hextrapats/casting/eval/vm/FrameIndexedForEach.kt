...
// Blatantly copied from FrameForEach, with an extra bit of logic to include the index

/**
 * A frame representing all the state for a Djehuty evaluation.
 * Pushed by SpecialHandlerIndexedForEach.
 * @property data list of *remaining* datums to IndexedForEach over
 ...
 * @property index the current index of the datum
 * @property acc concatenated list of final stack states after each iteration
 */
data class FrameIndexedForEach(
    ...
    override fun evaluate(
        ...
        val (newStack, newImage, newCont) = if (!data.isEmpty()) {
            ...
            val stack = contextStack.appended(data.head()).appended(DoubleIota(index.toDouble()))
            val cont2 = continuation
                // put the next Thoth object back on the stack for the next Thoth cycle,
                .pushFrame(FrameIndexedForEach(data.tail(), code, contextStack, stashedStack, index + 1, newAcc))
